"""
test_largecap.py — ICT + Price Action optimizer, cross-asset large-cap tech.

Usage:
    python strategy_ict/tests/test_largecap.py
    python strategy_ict/tests/test_largecap.py --jobs 4
"""
...
from strategy_ict import run_backtest

RESULTS_FILE = Path(__file__).parent.parent / "result_ict_largecap.csv"
...
def print_top(df: pd.DataFrame, all_data: dict, n: int = 10) -> None:
    print(f"\n{'─' * 140}")
    print(f"  TOP {n} ICT + PRICE ACTION COMBINATIONS  (score = mean_sharpe − 0.5×std_sharpe)")
    print(f"{'─' * 140}")
    cols = [c for c in [
        "ema_slow", "atr_sl", "atr_tp", "rsi_ob",
...
def main() -> None:
    p = argparse.ArgumentParser(description="ICT + Price Action — large-cap optimizer")
