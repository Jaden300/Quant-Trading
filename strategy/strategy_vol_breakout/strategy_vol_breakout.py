"""
strategy_vol_breakout.py — Volume Breakout backtest engine.

HOW IT WORKS:
  Buys when price closes above the highest high of the last N bars AND
  volume on that bar is significantly above average — confirming genuine
  institutional interest rather than a low-volume fake breakout.

Entry:
  close > max(high[last breakout_period bars])  — new N-bar high
  volume > vol_mult × avg(volume[last vol_period bars])  — volume spike
  → go long at next bar's open

Exit (whichever comes first):
  high >= entry * (1 + tp_pct)   — take profit hit intraday
  low  <= entry * (1 - sl_pct)   — stop loss hit intraday
  If both hit same bar → stop wins (conservative).

Execution: signal fires at bar close → entry/exit at NEXT bar's open.
TP/SL use intraday high/low. Gap-down through stop fills at open.
"""
...
def run_backtest(
    ...
    live_start = max(trade_start_idx, breakout_period + vol_period + 5)
    ...
    for i in range(1, n):
        ...
        if port.position == 0 and pending == 0 and i >= live_start:
            prev_high = float(np.max(high_a[i - breakout_period:i]))
            avg_vol   = float(np.mean(volume_a[i - vol_period:i]))

            if close_a[i] > prev_high and volume_a[i] > vol_mult * avg_vol:
                pending = 1
