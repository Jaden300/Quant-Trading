"""
test_largecap.py — Volume Breakout optimizer, cross-asset large-cap tech.

Usage:
    python strategy_vol_breakout/tests/test_largecap.py
    python strategy_vol_breakout/tests/test_largecap.py --jobs 4
"""
...
from strategy_vol_breakout import run_backtest

RESULTS_FILE = Path(__file__).parent.parent / "result_vol_breakout_largecap.csv"
...
PARAM_GRID: dict[str, list] = {
    ...
    "tp_pct":          [0.08, 0.12, 0.15, 0.20],
    "sl_pct":          [0.04, 0.06, 0.08],
}
...
def print_top(df: pd.DataFrame, all_data: dict, n: int = 10) -> None:
    print(f"\n{'─' * 130}")
    print(f"  TOP {n} VOLUME BREAKOUT COMBINATIONS  (score = mean_sharpe − 0.5×std_sharpe)")
    print(f"{'─' * 130}")
    cols = [c for c in [
        "breakout_period", "vol_period", "vol_mult", "tp_pct", "sl_pct",
    ...
    best = df.iloc[0]
    best_params = {k: (int(best[k]) if k in ("breakout_period", "vol_period") else float(best[k]))
                   for k in PARAM_GRID}
...
def main() -> None:
    p = argparse.ArgumentParser(description="Volume Breakout — large-cap optimizer")
