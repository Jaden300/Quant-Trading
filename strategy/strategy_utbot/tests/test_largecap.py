"""
test_largecap.py — UT Bot optimizer, cross-asset large-cap tech.

Usage:
    python strategy_utbot/tests/test_largecap.py
    python strategy_utbot/tests/test_largecap.py --jobs 4
"""
...
from strategy_utbot import run_backtest

RESULTS_FILE = Path(__file__).parent.parent / "result_utbot_largecap.csv"
...
PARAM_GRID: dict[str, list] = {
    "sensitivity": [0.5, 1.0, 1.5, 2.0, 3.0],
...
def print_top(df: pd.DataFrame, all_data: dict, n: int = 10) -> None:
    print(f"\n{'─' * 140}")
    print(f"  TOP {n} UT BOT COMBINATIONS  (score = mean_sharpe − 0.5×std_sharpe)")
    print(f"{'─' * 140}")
    cols = [c for c in [
        "sensitivity", "atr_period", "ema_slow", "rsi_buy", "adx_min",
...
def main() -> None:
    p = argparse.ArgumentParser(description="UT Bot — large-cap optimizer")
