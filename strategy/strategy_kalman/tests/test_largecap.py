"""
test_largecap.py — Dual Kalman Filter optimizer, cross-asset large-cap tech.

Usage:
    python strategy_kalman/tests/test_largecap.py
    python strategy_kalman/tests/test_largecap.py --jobs 4
"""
...
from strategy_kalman import run_backtest

RESULTS_FILE = Path(__file__).parent.parent / "result_kalman_largecap.csv"
...
PARAM_GRID: dict[str, list] = {
    "short_len": [2, 3, 5, 8],
    "long_len":  [30, 45, 65, 100],
    "kalman_r":  [0.02, 0.045, 0.1],
    "kalman_q":  [0.05, 0.15, 0.3],
}
...
def print_top(df: pd.DataFrame, all_data: dict, n: int = 10) -> None:
    print(f"\n{'─' * 140}")
    print(f"  TOP {n} KALMAN FILTER COMBINATIONS  (score = mean_sharpe − 0.5×std_sharpe)")
    print(f"{'─' * 140}")
    cols = [c for c in [
        "short_len", "long_len", "kalman_r", "kalman_q",
    ...
    best = df.iloc[0]
    best_params = {
        "short_len": int(best["short_len"]),
        "long_len":  int(best["long_len"]),
        "kalman_r":  float(best["kalman_r"]),
        "kalman_q":  float(best["kalman_q"]),
    }
...
def main() -> None:
    p = argparse.ArgumentParser(description="Kalman Filter — large-cap optimizer")
