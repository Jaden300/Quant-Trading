"""
test_largecap.py — Caldera Meridian Strategy optimizer, cross-asset large-cap tech.

Usage:
    python strategy_cms/tests/test_largecap.py
    python strategy_cms/tests/test_largecap.py --jobs 4
"""
...
from strategy_cms import run_backtest

RESULTS_FILE = Path(__file__).parent.parent / "result_cms_largecap.csv"
...
PARAM_GRID: dict[str, list] = {
    ...
    "min_score": [60.0, 72.0, 80.0],
    "risk_atr":  [1.5, 1.7, 2.5],
}
...
def print_top(df: pd.DataFrame, all_data: dict, n: int = 10) -> None:
    print(f"\n{'─' * 140}")
    print(f"  TOP {n} CMS COMBINATIONS  (score = mean_sharpe − 0.5×std_sharpe)")
    print(f"{'─' * 140}")
    cols = [c for c in [
        "fast_len", "mid_len", "slow_len", "min_score", "risk_atr",
    ...
    best = df.iloc[0]
    best_params = {
        "fast_len":  int(best["fast_len"]),
        "mid_len":   int(best["mid_len"]),
        "slow_len":  int(best["slow_len"]),
        "min_score": float(best["min_score"]),
        "risk_atr":  float(best["risk_atr"]),
    }
...
def main() -> None:
    p = argparse.ArgumentParser(description="Caldera Meridian Strategy — large-cap optimizer")
