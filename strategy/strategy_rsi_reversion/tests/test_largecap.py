"""
test_largecap.py — RSI Mean Reversion optimizer, cross-asset large-cap tech.

Usage:
    python strategy_rsi_reversion/tests/test_largecap.py
    python strategy_rsi_reversion/tests/test_largecap.py --jobs 4
"""
...
from strategy_rsi_reversion import run_backtest

RESULTS_FILE = Path(__file__).parent.parent / "result_rsi_reversion_largecap.csv"

DEFAULT_TICKERS = ["NVDA", "GOOG", "AAPL", "MSFT", "AMZN", "AVGO",
                   "META", "TSLA", "AMD", "NFLX"]
WARMUP_START    = "2020-07-01"
DEFAULT_START   = "2021-01-01"   # includes 2022 bear market — essential for RSI reversion
...
def print_top(df: pd.DataFrame, all_data: dict, n: int = 10) -> None:
    print(f"\n{'─' * 130}")
    print(f"  TOP {n} RSI REVERSION COMBINATIONS  (score = mean_sharpe − 0.5×std_sharpe)")
    print(f"{'─' * 130}")
    cols = [c for c in [
        "trend_ma", "rsi_length", "rsi_entry", "rsi_exit", "sl_pct",
    ...
    best = df.iloc[0]
    best_params = {k: (int(best[k]) if k in ("trend_ma","rsi_length") else float(best[k]))
                   for k in PARAM_GRID}
...
def main() -> None:
    p = argparse.ArgumentParser(description="RSI Mean Reversion — large-cap optimizer")
