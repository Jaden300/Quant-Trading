"""
test_largecap.py — ZLEMA Trailing Stop optimizer, cross-asset large-cap tech.

Usage:
    python strategy_zlema_trail/tests/test_largecap.py
    python strategy_zlema_trail/tests/test_largecap.py --jobs 4
"""
...
from strategy_zlema_trail import run_backtest

RESULTS_FILE = Path(__file__).parent.parent / "result_zlema_trail_largecap.csv"
...
PARAM_GRID: dict[str, list] = {
    "length":     [10, 15, 20, 30, 40],
    "mult":       [0.1, 0.2, 0.3, 0.5, 0.8],
    "ema_length": [10, 15, 20, 30, 50],
}
...
def print_top(df: pd.DataFrame, all_data: dict, n: int = 10) -> None:
    print(f"\n{'─' * 140}")
    print(f"  TOP {n} ZLEMA TRAIL COMBINATIONS  (score = mean_sharpe − 0.5×std_sharpe)")
    print(f"{'─' * 140}")
    cols = [c for c in [
        "length", "mult", "ema_length",
    ...
    best_params = {
        ...
        "mult":       float(best["mult"]),
        "ema_length": int(best["ema_length"]),
    }
...
def main() -> None:
    p = argparse.ArgumentParser(description="ZLEMA Trail — large-cap optimizer")
