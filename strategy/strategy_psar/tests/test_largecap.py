"""
test_largecap.py — Parabolic SAR optimizer, cross-asset large-cap tech.

Usage:
    python strategy_psar/tests/test_largecap.py
    python strategy_psar/tests/test_largecap.py --jobs 4
"""
...
from strategy_psar import run_backtest

RESULTS_FILE = Path(__file__).parent.parent / "result_psar_largecap.csv"
...
PARAM_GRID: dict[str, list] = {
    "start":     [0.01, 0.02, 0.03, 0.05],
    "increment": [0.01, 0.02, 0.03],
    "maximum":   [0.10, 0.20, 0.30],
    "tp_pct":    [0.0, 0.12, 0.20],
}
...
def print_top(df: pd.DataFrame, all_data: dict, n: int = 10) -> None:
    print(f"\n{'─' * 130}")
    print(f"  TOP {n} PARABOLIC SAR COMBINATIONS  (score = mean_sharpe − 0.5×std_sharpe)")
    print(f"{'─' * 130}")
    cols = [c for c in [
        "start", "increment", "maximum", "tp_pct",
    ...
    best = df.iloc[0]
    best_params = {k: float(best[k]) for k in PARAM_GRID}
...
def main() -> None:
    p = argparse.ArgumentParser(description="Parabolic SAR — large-cap optimizer")
