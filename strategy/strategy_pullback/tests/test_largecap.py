"""
test_largecap.py — Pullback strategy optimizer, cross-asset across large-cap tech stocks.

Tickers: NVDA, GOOG, AAPL, MSFT, AMZN, AVGO
Ranks by score = mean_sharpe − 0.5 × std_sharpe  (balances performance + consistency).

Usage:
    python strategy_pullback/tests/test_largecap.py
    python strategy_pullback/tests/test_largecap.py --jobs 4
    python strategy_pullback/tests/test_largecap.py --tickers TSLA NVDA AAPL
"""
...
from strategy_pullback import run_backtest

RESULTS_FILE = Path(__file__).parent.parent / "result_pullback_largecap.csv"
...
PARAM_GRID: dict[str, list] = {
    ...
    "trend_threshold":  [0.02, 0.05, 0.08],
    "dip_threshold":    [0.02, 0.05, 0.08],
    "stop_buffer":      [0.005, 0.01, 0.02],
    "tp_pct":           [0.08, 0.12, 0.18, 0.25],
...
def print_top(df: pd.DataFrame, all_data: dict, n: int = 10) -> None:
    ...
    print(f"  TOP {n} CROSS-ASSET COMBINATIONS  "
          f"(score = mean_sharpe − 0.5×std_sharpe)")
    print(f"{'─' * 140}")
    cols = [c for c in [
        "long_period", "short_period", "trend_threshold", "dip_threshold",
        "stop_buffer", "tp_pct", "sl_pct",
    ...
    print(top.to_string(float_format=lambda x: f"{x:7.3f}", max_colwidth=14))
    print(f"{'─' * 140}\n")
...
def _row_to_params(row: pd.Series) -> dict:
    return {
        ...
        "dip_threshold":   float(row["dip_threshold"]),
...
def main() -> None:
    p = argparse.ArgumentParser(description="Pullback strategy — large-cap cross-asset optimizer")
