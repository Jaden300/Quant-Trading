"""
test_largecap.py — Gap Fill optimizer, cross-asset large-cap tech.

Usage:
    python strategy_gap_fill/tests/test_largecap.py
    python strategy_gap_fill/tests/test_largecap.py --jobs 4
"""
...
from strategy_gap_fill import run_backtest

RESULTS_FILE = Path(__file__).parent.parent / "result_gap_fill_largecap.csv"
...
PARAM_GRID: dict[str, list] = {
    "min_gap_pct": [0.02, 0.03, 0.05, 0.07],
    "sl_pct":      [0.04, 0.06, 0.08, 0.12],
    "max_hold":    [1, 3, 5, 10],
}
...
def print_top(df: pd.DataFrame, all_data: dict, n: int = 10) -> None:
    print(f"\n{'─' * 130}")
    print(f"  TOP {n} GAP FILL COMBINATIONS  (score = mean_sharpe − 0.5×std_sharpe)")
    print(f"{'─' * 130}")
    cols = [c for c in [
        "min_gap_pct", "sl_pct", "max_hold",
    ...
    best = df.iloc[0]
    best_params = {k: (int(best[k]) if k == "max_hold" else float(best[k]))
                   for k in PARAM_GRID}
...
def main() -> None:
    p = argparse.ArgumentParser(description="Gap Fill — large-cap optimizer")
