"""
test_largecap.py — Regime Execution Strategy optimizer, cross-asset large-cap tech.

Usage:
    python strategy_regime/tests/test_largecap.py
    python strategy_regime/tests/test_largecap.py --jobs 4
"""
...
RESULTS_FILE = Path(__file__).parent.parent / "result_regime_largecap.csv"
...
PARAM_GRID: dict[str, list] = {
    "lambda_f":   [0.95, 0.975, 0.99],
    "band_mult":  [1.2, 1.8, 2.5],
    "stop_atr":   [1.2, 1.6, 2.2],
    "target_atr": [2.0, 2.6, 3.5],
}
...
def print_top(df: pd.DataFrame, all_data: dict, n: int = 10) -> None:
    print(f"\n{'─' * 140}")
    print(f"  TOP {n} REGIME EXECUTION COMBINATIONS  (score = mean_sharpe − 0.5×std_sharpe)")
    print(f"{'─' * 140}")
    cols = [c for c in [
        "lambda_f", "band_mult", "stop_atr", "target_atr",
    ...
    best_params = {
        ...
        "band_mult":  float(best["band_mult"]),
        "stop_atr":   float(best["stop_atr"]),
        "target_atr": float(best["target_atr"]),
    }
...
def main() -> None:
    p = argparse.ArgumentParser(description="Regime Execution — large-cap optimizer")
