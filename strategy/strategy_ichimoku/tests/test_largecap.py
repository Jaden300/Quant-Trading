"""
test_largecap.py — Ichimoku Cloud optimizer, cross-asset large-cap tech.

Usage:
    python strategy_ichimoku/tests/test_largecap.py
    python strategy_ichimoku/tests/test_largecap.py --jobs 4
"""
...
from strategy_ichimoku import run_backtest

RESULTS_FILE = Path(__file__).parent.parent / "result_ichimoku_largecap.csv"
...
PARAM_GRID: dict[str, list] = {
    ...
    "use_chikou":      [True, False],
    "sl_pct":          [0.0, 0.08, 0.15],
}
...
def print_top(df: pd.DataFrame, all_data: dict, n: int = 10) -> None:
    print(f"\n{'─' * 140}")
    print(f"  TOP {n} ICHIMOKU COMBINATIONS  (score = mean_sharpe − 0.5×std_sharpe)")
    print(f"{'─' * 140}")
    cols = [c for c in [
        "tenkan_period", "kijun_period", "senkou_b_period", "use_cloud", "use_chikou", "sl_pct",
    ...
    best = df.iloc[0]
    best_params = {
        "tenkan_period":   int(best["tenkan_period"]),
        "kijun_period":    int(best["kijun_period"]),
        "senkou_b_period": int(best["senkou_b_period"]),
        "use_cloud":       bool(best["use_cloud"]),
        "use_chikou":      bool(best["use_chikou"]),
...
def main() -> None:
    p = argparse.ArgumentParser(description="Ichimoku Cloud — large-cap optimizer")
