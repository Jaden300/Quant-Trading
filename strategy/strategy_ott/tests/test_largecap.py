"""
test_largecap.py — OTT optimizer, cross-asset large-cap tech.

Usage:
    python strategy_ott/tests/test_largecap.py
    python strategy_ott/tests/test_largecap.py --jobs 4
"""
...
from strategy_ott import run_backtest

RESULTS_FILE = Path(__file__).parent.parent / "result_ott_largecap.csv"
...
PARAM_GRID: dict[str, list] = {
    "length":  [2, 3, 5, 8],
    "percent": [1.0, 1.4, 2.0, 3.0],
    "mav":     ["VAR", "EMA", "WWMA"],
    "tp_pct":  [0.0, 0.15, 0.25],
}
...
def print_top(df: pd.DataFrame, all_data: dict, n: int = 10) -> None:
    print(f"\n{'─' * 130}")
    print(f"  TOP {n} OTT COMBINATIONS  (score = mean_sharpe − 0.5×std_sharpe)")
    print(f"{'─' * 130}")
    cols = [c for c in [
        "length", "percent", "mav", "tp_pct",
...
def main() -> None:
    p = argparse.ArgumentParser(description="OTT — large-cap optimizer")
