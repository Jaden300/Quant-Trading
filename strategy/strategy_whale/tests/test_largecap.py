"""
test_largecap.py — Whale Force optimizer, cross-asset large-cap tech.

Usage:
    python strategy_whale/tests/test_largecap.py
    python strategy_whale/tests/test_largecap.py --jobs 4
"""
...
from strategy_whale import run_backtest

RESULTS_FILE = Path(__file__).parent.parent / "result_whale_largecap.csv"
...
PARAM_GRID: dict[str, list] = {
    "z_thresh":       [1.8, 2.2, 2.8],
    "don_len":        [15, 24, 35],
    "quiet_ratio":    [0.65, 0.75, 0.90],
    "min_exit_votes": [1, 2, 3],
}
...
def print_top(df: pd.DataFrame, all_data: dict, n: int = 10) -> None:
    print(f"\n{'─' * 140}")
    print(f"  TOP {n} WHALE FORCE COMBINATIONS  (score = mean_sharpe − 0.5×std_sharpe)")
    print(f"{'─' * 140}")
    cols = [c for c in [
        "z_thresh", "don_len", "quiet_ratio", "min_exit_votes",
    ...
    best_params = {
        ...
        "don_len":        int(best["don_len"]),
        "quiet_ratio":    float(best["quiet_ratio"]),
        "min_exit_votes": int(best["min_exit_votes"]),
    }
...
def main() -> None:
    p = argparse.ArgumentParser(description="Whale Force — large-cap optimizer")
