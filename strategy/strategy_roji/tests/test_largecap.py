"""
test_largecap.py — Roji Pattern optimizer (RBR / DBR / Double Bottom), cross-asset.

Usage:
    python strategy_roji/tests/test_largecap.py
    python strategy_roji/tests/test_largecap.py --jobs 4
"""
...
from strategy_roji import run_backtest

RESULTS_FILE = Path(__file__).parent.parent / "result_roji_largecap.csv"
...
PARAM_GRID: dict[str, list] = {
    "base_bars":    [2, 3, 5],
    "impulse_mult": [0.5, 0.8, 1.2],
    "base_mult":    [0.5, 0.8, 1.2],
    "atr_length":   [10, 14, 20],
    "rr":           [1.5, 2.0, 2.5],
}
...
def print_top(df: pd.DataFrame, all_data: dict, n: int = 10) -> None:
    print(f"\n{'─' * 140}")
    print(f"  TOP {n} ROJI PATTERN COMBINATIONS  (score = mean_sharpe − 0.5×std_sharpe)")
    print(f"{'─' * 140}")
    cols = [c for c in [
        "base_bars", "impulse_mult", "base_mult", "atr_length", "rr",
    ...
    best = df.iloc[0]
    best_params = {
        "base_bars":    int(best["base_bars"]),
        "impulse_mult": float(best["impulse_mult"]),
        "base_mult":    float(best["base_mult"]),
...
def main() -> None:
    p = argparse.ArgumentParser(description="Roji Pattern — large-cap optimizer")
