"""
test_largecap.py — Lucky 13 EMA optimizer, cross-asset large-cap tech.

Usage:
    python strategy_lucky13/tests/test_largecap.py
    python strategy_lucky13/tests/test_largecap.py --jobs 4
"""
...
from strategy_lucky13 import run_backtest

RESULTS_FILE = Path(__file__).parent.parent / "result_lucky13_largecap.csv"
...
def print_top(df: pd.DataFrame, all_data: dict, n: int = 10) -> None:
    print(f"\n{'─' * 130}")
    print(f"  TOP {n} LUCKY 13 COMBINATIONS  (score = mean_sharpe − 0.5×std_sharpe)")
    print(f"{'─' * 130}")
    cols = [c for c in [
        "ema_length", "vol_mult", "tp_pct", "sl_pct",
    ...
    best = df.iloc[0]
    best_params = {k: (int(best[k]) if k == "ema_length" else float(best[k]))
                   for k in PARAM_GRID}
...
def main() -> None:
    p = argparse.ArgumentParser(description="Lucky 13 EMA — large-cap optimizer")
